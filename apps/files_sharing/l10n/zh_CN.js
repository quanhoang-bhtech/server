OC.L10N.register(
    ...
    "Failed to add the public link to your xD Drive" : "无法添加公开链接到您的 xD Drive",
    ...
    "Sharing %s sending the password by xD Drive Talk failed because xD Drive Talk is not enabled" : "由于 xD Drive Talk 没有启用，所以通过 xD Drive Talk 发送 %s 共享密码失败。",
    ...
    "\"Sending the password by xD Drive Talk\" for sharing a file or folder failed because xD Drive Talk is not enabled." : "“通过xD Drive Talk发送密码”共享文件或文件夹失败了，因为未启用 xD Drive Talk",
    "shared by %s" : "共享者 %s",
    ...
    "Accept" : "接受",
    "Reject" : "拒绝",
    "This application enables users to share files within xD Drive. If enabled, the admin can choose which groups can share files. The applicable users can then share files and folders with other users and groups within xD Drive. In addition, if the admin enables the share link feature, an external link can be used to share files with other users outside of xD Drive. Admins can also enforce passwords, expirations dates, and enable server to server sharing via share links, as well as sharing from mobile devices.\nTurning the feature off removes shared files and folders on the server for all share recipients, and also on the sync clients and mobile apps. More information is available in the xD Drive Documentation." : "这个应用程序允许用户在 xD Drive 内部共享文件。如果启用，管理员能够选择哪个组可以共享文件。然后，可用的用户能够共享文件和文件夹给 xD Drive 中的其他用户和组。另外，如果管理员启用了共享链接功能，能够使用外部链接将文件共享给 xD Drive 之外的其他用户。管理员也可以强制设置密码，过期日期，并且可以通过共享链接启用服务器到服务器的共享，以及从移动设备共享。\n关闭该功能将删除服务器上所有共享接收者以用同步客户端和移动应用上的共享文件和文件夹。更多信息可查阅 xD Drive 文档。",
    "Sharing" : "共享",
    ...
    "Add to your xD Drive" : "添加到您的 xD Drive",
