		...
		switch (get_class($this->connection->getDatabasePlatform())) {
			...
			case MySQLPlatform::class:
				...
				if (strpos($version, 'mariadb') !== false) {
					if (version_compare($version, '10.2', '<')) {
						$this->description = $this->l10n->t('MariaDB version "%s" is used. xD Drive 21 and higher do not support this version and require MariaDB 10.2 or higher.', $row['Value']);
						return;
					}
				} else {
					if (version_compare($version, '8', '<')) {
						$this->description = $this->l10n->t('MySQL version "%s" is used. xD Drive 21 and higher do not support this version and require MySQL 8.0 or MariaDB 10.2 or higher.', $row['Value']);
						return;
					}
				}
				break;
			...
			case PostgreSQL94Platform::class:
				...
				if (version_compare($row['server_version'], '9.6', '<')) {
					$this->description = $this->l10n->t('PostgreSQL version "%s" is used. xD Drive 21 and higher do not support this version and require PostgreSQL 9.6 or higher.', $row['server_version']);
					return;
				}
				break;
