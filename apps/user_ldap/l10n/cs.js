OC.L10N.register(
    ...
    "This application enables administrators to connect xD Drive to an LDAP-based user directory." : "Tato aplikace umožňuje správcům připojit xD Drive na adresář uživatelů založený na LDAP.",
    "This application enables administrators to connect xD Drive to an LDAP-based user directory for authentication and provisioning users, groups and user attributes. Admins can configure this application to connect to one or more LDAP directories or Active Directories via an LDAP interface. Attributes such as user quota, email, avatar pictures, group memberships and more can be pulled into xD Drive from a directory with the appropriate queries and filters.\n\nA user logs into xD Drive with their LDAP or AD credentials, and is granted access based on an authentication request handled by the LDAP or AD server. xD Drive does not store LDAP or AD passwords, rather these credentials are used to authenticate a user and then xD Drive uses a session for the user ID. More information is available in the LDAP User and Group Backend documentation." : "Tato aplikace umožní správcům propojit xD Drive s adresářem uživatelů, založeném na LDAP pro ověřování a zprovozňování uživatelů, skupin a atributů uživatelů. Správci mohou tuto aplikaci nastavit pro propojení s jedním či více LDAP adresáři nebo Active Directory prostřednictvím LDAP rozhraní. Atributy jako například kvóta uživatele, e-mail, fotografie, členství ve skupinách a další mohou být vytažené do xD Drive z adresáře pomocí příslušných dotazů a filtrů.\n\nUživatel se do xD Drive přihlásí pomocí svých LDAP nebo AD přihlašovacích údajů a je mu udělen přístup na základě požadavku na ověření obslouženém LDAP nebo AD serverem. xD Drive neukládá LDAP nebo AD hesla, namísto toho jsou tyto přihlašovací údaje použity pro ověření uživatele a xD Drive používá relaci pro identifikátor  uživatele. Více informací je k dispozici v dokumentaci k podpůrné vrstvě LDAP uživatel a skupina.",
