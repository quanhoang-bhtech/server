OC.L10N.register(
    ...
    "This application enables administrators to connect xD Drive to an LDAP-based user directory." : "Aplikazio honi esker administratzaileek xD Drive konektatu dezakete LDAPn oinarritutako erabiltzaile direktorio batera.",
    "This application enables administrators to connect xD Drive to an LDAP-based user directory for authentication and provisioning users, groups and user attributes. Admins can configure this application to connect to one or more LDAP directories or Active Directories via an LDAP interface. Attributes such as user quota, email, avatar pictures, group memberships and more can be pulled into xD Drive from a directory with the appropriate queries and filters.\n\nA user logs into xD Drive with their LDAP or AD credentials, and is granted access based on an authentication request handled by the LDAP or AD server. xD Drive does not store LDAP or AD passwords, rather these credentials are used to authenticate a user and then xD Drive uses a session for the user ID. More information is available in the LDAP User and Group Backend documentation." : "Aplikazio honek administratzaileei aukera ematen die xD Drive LDAP oinarritutako erabiltzaile direktorio batera konektatzeko, erabiltzaile, talde eta erabiltzaile atributuak autentifikatzeko eta hornitzeko. Administratzaileek aplikazio hau konfiguratu dezakete LDAP direktorio edo direktorio aktibo batera edo gehiagora konektatzeko LDAP interfaze baten bidez. Erabiltzaileen kuota, posta elektronikoa, avatarren argazkiak, taldeko kidetasunak eta halako atributu gehiago xD Drive-era atera daitezke kontsulta eta iragazki egokiak dituen direktorio batetik.\n\nErabiltzaile batek xD Drive-en saioa hasten du bere LDAP edo AD egiaztagiriekin, eta sarbidea ematen zaio LDAP edo AD zerbitzariak kudeatzen duen autentifikazio eskaera oinarritzat hartuta. xD Drive-ek ez ditu LDAP edo AD pasahitzak gordetzen, kredentzial hauek erabiltzaile bat autentifikatzeko erabiltzen dira eta ondoren xD Drive-ek saio bat erabiltzen du erabiltzaile IDarentzat. Informazio gehiago eskuragarri dago LDAP Erabiltzaile eta Taldearen atzealdeko dokumentazioan.",
