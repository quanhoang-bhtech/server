OC.L10N.register(
    ...
    "This application enables administrators to connect xD Drive to an LDAP-based user directory." : "Questa applicazione consente agli amministratori di collegare xD Drive a una directory di utenti basata su LDAP.",
    "This application enables administrators to connect xD Drive to an LDAP-based user directory for authentication and provisioning users, groups and user attributes. Admins can configure this application to connect to one or more LDAP directories or Active Directories via an LDAP interface. Attributes such as user quota, email, avatar pictures, group memberships and more can be pulled into xD Drive from a directory with the appropriate queries and filters.\n\nA user logs into xD Drive with their LDAP or AD credentials, and is granted access based on an authentication request handled by the LDAP or AD server. xD Drive does not store LDAP or AD passwords, rather these credentials are used to authenticate a user and then xD Drive uses a session for the user ID. More information is available in the LDAP User and Group Backend documentation." : "Questa applicazione consente agli amministratori di collegare xD Drive a una directory di utenti basata su LDAP per l'autenticazione e la creazione di utenti, gruppi e attributi utente. Gli amministratori possono configurare questa applicazione per collegare una o più directory LDAP o Active Directory tramite un'interfaccia LDAP. Gli attributi come quota utente, email, immagini personali, appartenenza ai gruppi e altro possono essere portati in xD Drive da una directory con le interrogazioni e i filtri appropriati.\n\nUn utente accede a xD Drive con le proprie credenziali LDAP o AD, e l'accesso viene accordato sulla base della richiesta di autenticazione gestita dal server LDAP o AD. xD Drive non memorizza le password LDAP o AD, piuttosto queste credenziali sono utilizzate per autenticare un utente e poi xD Drive utilizza una sessione per l'ID utente. Altre informazioni sono disponibili nella documentazione relativa a Motore utenti e gruppi LDAP.",
