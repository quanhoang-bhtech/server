OC.L10N.register(
    ...
    "This application enables administrators to connect xD Drive to an LDAP-based user directory." : "このアプリケーションを使用すると、管理者はLDAPベースのユーザーディレクトリにxD Driveを接続できます。",
    "This application enables administrators to connect xD Drive to an LDAP-based user directory for authentication and provisioning users, groups and user attributes. Admins can configure this application to connect to one or more LDAP directories or Active Directories via an LDAP interface. Attributes such as user quota, email, avatar pictures, group memberships and more can be pulled into xD Drive from a directory with the appropriate queries and filters.\n\nA user logs into xD Drive with their LDAP or AD credentials, and is granted access based on an authentication request handled by the LDAP or AD server. xD Drive does not store LDAP or AD passwords, rather these credentials are used to authenticate a user and then xD Drive uses a session for the user ID. More information is available in the LDAP User and Group Backend documentation." : "このアプリケーションを使用すると、管理者は認証、プロビジョニングのためにLDAPベースのユーザーディレクトリにxD Driveを接続し、ユーザー、グループ、およびユーザー属性をプロビジョニングできます。 管理者は、LDAPインターフェイス経由で1つまたは複数のLDAPディレクトリまたはActive Directoryに接続するようにこのアプリケーションを設定できます。 ユーザークオータ、電子メール、アバターピクチャ、グループメンバーシップなどの属性は、適切なクエリとフィルタを使用してディレクトリからxD Driveに取り込むことができます。ユーザーはLDAPまたはAD資格情報でxD Driveにログインし、 LDAPまたはADサーバーによって処理される認証要求。 xD DriveはLDAPまたはADのパスワードを格納しませんが、これらの資格情報はユーザーの認証に使用され、xD DriveはユーザーIDのセッションを使用します。 詳細は、LDAPユーザーおよびグループのバックエンドのドキュメントを参照してください。",
