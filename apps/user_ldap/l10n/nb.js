OC.L10N.register(
    ...
    "This application enables administrators to connect xD Drive to an LDAP-based user directory." : "Denne applikasjonen lar administratorer koble xD Drive til en LDAP-basert brukerkatalog.",
    "This application enables administrators to connect xD Drive to an LDAP-based user directory for authentication and provisioning users, groups and user attributes. Admins can configure this application to connect to one or more LDAP directories or Active Directories via an LDAP interface. Attributes such as user quota, email, avatar pictures, group memberships and more can be pulled into xD Drive from a directory with the appropriate queries and filters.\n\nA user logs into xD Drive with their LDAP or AD credentials, and is granted access based on an authentication request handled by the LDAP or AD server. xD Drive does not store LDAP or AD passwords, rather these credentials are used to authenticate a user and then xD Drive uses a session for the user ID. More information is available in the LDAP User and Group Backend documentation." : "Denne applikasjonen lar administratorer koble xD Drive til en LDAP-basert brukerkatalog for autentisering og klargjøring av brukere, grupper og brukerattributter. Administratorer kan konfigurere dette programmet for å koble til en eller flere LDAP-kataloger eller Active Directory via et LDAP-grensesnitt. Attributter som brukerkvote, e-post, avatarbilder, gruppemedlemskap og mer kan trekkes inn i xD Drive fra en katalog med riktige spørsmål og filtre.\n\nEn bruker logger seg på xD Drive med LDAP- eller AD-legitimasjon, og får tilgang basert på en autentiseringsforespørsel håndtert av LDAP- eller AD-serveren. xD Drive lagrer ikke LDAP- eller AD-passord, men disse legitimasjonene brukes til å autentisere en bruker, og deretter bruker xD Drive en økt for bruker-ID-en. Mer informasjon er tilgjengelig i LDAP User and Group Backend-dokumentasjonen.",
