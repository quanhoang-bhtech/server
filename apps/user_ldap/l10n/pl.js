OC.L10N.register(
    ...
    "This application enables administrators to connect xD Drive to an LDAP-based user directory." : "Aplikacja umożliwia administratorom podłączenie xD Drive do katalogu użytkownika opartego na LDAP.",
    "This application enables administrators to connect xD Drive to an LDAP-based user directory for authentication and provisioning users, groups and user attributes. Admins can configure this application to connect to one or more LDAP directories or Active Directories via an LDAP interface. Attributes such as user quota, email, avatar pictures, group memberships and more can be pulled into xD Drive from a directory with the appropriate queries and filters.\n\nA user logs into xD Drive with their LDAP or AD credentials, and is granted access based on an authentication request handled by the LDAP or AD server. xD Drive does not store LDAP or AD passwords, rather these credentials are used to authenticate a user and then xD Drive uses a session for the user ID. More information is available in the LDAP User and Group Backend documentation." : "Aplikacja umożliwia administratorom łączenie xD Drive z katalogiem użytkowników opartym na LDAP w celu uwierzytelniania i udostępniania użytkowników, grup i atrybutów użytkowników. Administratorzy mogą skonfigurować tę aplikację do łączenia się z jednym lub większą liczbą katalogów LDAP lub katalogów aktywnych za pośrednictwem interfejsu LDAP. Atrybuty, takie jak limity użytkowników, wiadomości e-mail, zdjęcia awatarów, członkostwa w grupach i inne, można pobrać do xD Drive z katalogu z odpowiednimi zapytaniami i filtrami.\n\nUżytkownik loguje się do xD Drive za pomocą swoich poświadczeń LDAP lub AD i uzyskuje dostęp na podstawie żądania uwierzytelnienia obsługiwanego przez serwer LDAP lub AD. xD Drive nie przechowuje haseł LDAP lub AD, poświadczenia te są użyte do uwierzytelnienia użytkownika. Następnie xD Drive używa sesji dla ID użytkownika. Więcej informacji można znaleźć w dokumentacji użytkownika LDAP i zaplecza grupy.",
