OC.L10N.register(
    ...
    "This application enables administrators to connect xD Drive to an LDAP-based user directory." : "Este aplicativo permite que os administradores conectem o xD Drive a um diretório de usuários baseado em LDAP.",
    "This application enables administrators to connect xD Drive to an LDAP-based user directory for authentication and provisioning users, groups and user attributes. Admins can configure this application to connect to one or more LDAP directories or Active Directories via an LDAP interface. Attributes such as user quota, email, avatar pictures, group memberships and more can be pulled into xD Drive from a directory with the appropriate queries and filters.\n\nA user logs into xD Drive with their LDAP or AD credentials, and is granted access based on an authentication request handled by the LDAP or AD server. xD Drive does not store LDAP or AD passwords, rather these credentials are used to authenticate a user and then xD Drive uses a session for the user ID. More information is available in the LDAP User and Group Backend documentation." : "Esse aplicativo permite que os administradores conectem o xD Drive a um diretório de usuários baseado em LDAP para autenticação, provisionamento de usuários, grupos e atributos do usuário. Os administradores podem configurar este aplicativo para se conectar a um ou mais diretórios LDAP ou Active Directories por meio de uma interface LDAP. Atributos como cota de usuário, e-mail, imagens de avatar, associações a grupos e muito mais podem ser acessados no xD Drive a partir de um diretório com as consultas e filtros apropriados.\n\nUm usuário efetua login no xD Drive com suas credenciais LDAP ou AD e recebe acesso com base em uma solicitação de autenticação manipulada pelo servidor LDAP ou AD. O xD Drive não armazena senhas LDAP ou AD, mas essas credenciais são usadas para autenticar o usuário e em seguida, o xD Drive usa uma sessão para o ID do usuário. Mais informações estão disponíveis na documentação da estrutura LDAP de  Usuário e Grupo.",
