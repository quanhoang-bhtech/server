OC.L10N.register(
    ...
    "This application enables administrators to connect xD Drive to an LDAP-based user directory." : "Aplikácia umožňuje administrátorom pripojenie xD Drive k užívateľskému adresáru LDAP.",
    "This application enables administrators to connect xD Drive to an LDAP-based user directory for authentication and provisioning users, groups and user attributes. Admins can configure this application to connect to one or more LDAP directories or Active Directories via an LDAP interface. Attributes such as user quota, email, avatar pictures, group memberships and more can be pulled into xD Drive from a directory with the appropriate queries and filters.\n\nA user logs into xD Drive with their LDAP or AD credentials, and is granted access based on an authentication request handled by the LDAP or AD server. xD Drive does not store LDAP or AD passwords, rather these credentials are used to authenticate a user and then xD Drive uses a session for the user ID. More information is available in the LDAP User and Group Backend documentation." : "Táto aplikácia umožňuje administrátorom prepojiť xD Drive s priečinkami používateľov založených na LDAP pre overovanie a sprevádzkovanie používateľov, skupín a atribútov používateľov. Správcovia môžu nastaviť túto aplikáciu na pripojenie k jednému alebo viacerým priečinkom LDAP alebo Active Directory prostredníctvom LDAP. Atribúty, ako sú užívateľská kvóta, e-mail, fotografie, členstvo v skupinách a ďalšie, sa dajú do xD Drive vytiahnuť z priečinka príslušnými požiadavkami a filtrami.\n\nUžívateľ sa prihlási do systému xD Drive svojimi prihlasovacími údajmi LDAP alebo AD a prístup mu bude udelený na základe žiadosti o overenie totožnosti vybavenej serverom LDAP alebo AD. xD Drive neuchováva heslá LDAP ani AD, ale tieto poverenia sa používajú na overenie totožnosti používateľa a potom xD Drive použije reláciu pre ID používateľa. Viac informácií je k dispozícii v dokumentácii k podpornej vrstve LDAP používateľ a skupina.",
