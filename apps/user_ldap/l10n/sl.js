OC.L10N.register(
    ...
    "This application enables administrators to connect xD Drive to an LDAP-based user directory." : "Program omogoča skrbnikom vzpostavljanje povezave med okoljem xD Drive in uporabniškim imenikom LDAP.",
    "This application enables administrators to connect xD Drive to an LDAP-based user directory for authentication and provisioning users, groups and user attributes. Admins can configure this application to connect to one or more LDAP directories or Active Directories via an LDAP interface. Attributes such as user quota, email, avatar pictures, group memberships and more can be pulled into xD Drive from a directory with the appropriate queries and filters.\n\nA user logs into xD Drive with their LDAP or AD credentials, and is granted access based on an authentication request handled by the LDAP or AD server. xD Drive does not store LDAP or AD passwords, rather these credentials are used to authenticate a user and then xD Drive uses a session for the user ID. More information is available in the LDAP User and Group Backend documentation." : "Program omogoča skrbnikom, da povežejo okolje xD Drive z uporabniškim imenikom LDAP za overjanje pristnosti in zagotavljanje dostopa do podatkov uporabnikov, atributov in skupin. Skrbniki lahko nastavijo program za povezavo z enim ali več imeniki LDAP ali AD prek vmesnika LDAP. Atribute, kot so količinska omejitev, elektronski naslov, sličice profila, podatki o članstvu in drugo, je mogoče pridobiti iz imenika z ustreznimi poizvedbami in filtri.\n\nUporabnik se prijavi v xD Drive z ustreznimi poverili LDAP in AD in si pridobi dostop na podlagi zahteve za preverjanje pristnosti, ki jo strežnik obdela. Okolje xD Drive ne shranjuje gesel LDAP ali AD, temveč se ta poverila uporabljajo za preverjanje pristnosti uporabnika, nato pa xD Drive uporabi sejo za povezan ID uporabnika. Več podrobnosti je na voljo v dokumentaciji ozadnjega programa LDAP za uporabnike in skupine.\n \n ",
